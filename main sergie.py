...
def main():
    ...
    my_res, rival_res, differences = [], [], []
    for i in range(5):
        print(f'Game No.{i+1}')
        game = Game(an_input)
        results = game.play_game()
        my_res.append(results[0])
        rival_res.append(results[1])
        differences.append(results[0]-results[1])
        print(f'Score for {exp3.IDS} is {results[0]}, score for {sample_agent.IDS} is {results[1]}')
    plt.plot(my_res, label='my')
    plt.plot(rival_res, label='rival')
    plt.plot(differences, label='difference')
    # Calculate means
    mean_my_res = np.mean(my_res)
    mean_rival_res = np.mean(rival_res)
    mean_differences = np.mean(differences)

    # Adding mean lines
    plt.axhline(y=mean_my_res, color='blue', linestyle='-', linewidth=0.8, label=f'Mean My Results {mean_my_res}')
    plt.axhline(y=mean_rival_res, color='orange', linestyle='-', linewidth=0.8, label=f'Mean Rival Results {mean_rival_res}')
    plt.axhline(y=mean_differences, color='green', linestyle='-', linewidth=0.8, label=f'Mean Differences {mean_differences}')
